test_that("subgroups outputs the right element for survival", {
  result <- subgroups(elastic_net_fit_surv$design_matrix,
  ...
  expected <- data.frame(
    ...
    trt.estimate = c(0.65, 0.6494, 0.6494, 0.6502, 0.6498, 0.649, 0.6474,
...
test_that("subgroups outputs the right element for binary", {
  result <- subgroups(elastic_net_fit_bin$design_matrix,
                      elastic_net_fit_bin$design_dummy,
                      elastic_net_fit_bin$subgr_names,
                      "binary", est_coef_bin1)
  expected <- data.frame(
    subgroup = elastic_net_fit_surv$subgr_names,
    trt.estimate = c(-0.463, -0.4641, -0.4641, -0.4627, -0.4641, -0.4656,
