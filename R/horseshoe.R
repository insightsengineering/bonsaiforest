...
horseshoe <- function(resp, trt, subgr, covars, data,
  ...
  form_b <- stats::as.formula(paste("b ~ 0 +",
  ...
  if (resptype == "survival") {
    ...
    form_a_surv <- stats::as.formula(paste("a ~ 0 +",
    ...
    bhaz <- list(Boundary.knots = limits_resp, knots = quantiles_resp,
                 intercept = FALSE)
    fit_brms <- brms::brm(brms::bf(form_surv, nl = TRUE) +
                            brms::lf(form_a_surv) +
                            brms::lf(form_b),
                          data = data_model,
                          family = brms::brmsfamily("cox", bhaz = bhaz),
                          brms::prior(normal(0, 5), class = "b", nlpar = "a") +
                            brms::prior(horseshoe(1), class = "b",
                                        nlpar = "b"),
    ...
    colnames(y) <- resp
    data_model <- cbind(design_matrix, y)
    fit_brms <- brms::brm(brms::bf(form_bin, nl = TRUE) + brms::lf(form_a_bin) +
                            brms::lf(form_b),
                          data = data_model,
