...
preproc <- function(trt, subgr, covars, data){
  ...
  list(design_ia = design_ia, design_main = design_main,
...
elastic_net <- function(resp, trt, subgr, covars, status = NULL, data, resptype, alpha){
  ...
  if (resptype == "survival"){
    ...
    fit_glmnet <- cv.glmnet(design_matrix, data[[resp]], family = "binomial",
                            penalty.factor = penalty_factor, alpha = alpha)
    y <- data[[resp]]
  }
  list(fit = fit_glmnet, alpha = alpha, model = "elastic_net",
...
summary_en <- function(obj_en){
  if (obj_en$resptype == "survival"){
    result <- trt_en(obj_en$design_matrix, obj_en$design_dummy, obj_en$y,
                     obj_en$subgr_names, obj_en$resptype, obj_en$fit, gamma=1)
...
trt_en <- function(X, X_dummy, y, subgr_names, resptype, fit, gamma = NULL, L = NULL){
  ...
  if (resptype == "binary"){
    ...
    bh <- basehaz.gbm(t = resp_un, delta = status_un, f.x = lp_un, t.eval = resp_un,
    ...
    list <- list(H0 = bh[ind_time], est_coef = est_coef,
                 gamma = gamma)
...
AHR_funct <- function(X_subg, dummy_subg, list){
  with(list,{
    # Data for treatment
    X1 <- cbind(rep(0, nrow(X_subg)), rep(1, nrow(X_subg)),
...
lasso1_HR1 <- matrix(NA,nrow=25,ncol=n_datasets)

for (i in 1:n_datasets){
  data1 = scenario1[[i]]
  data1$arm <- as.factor(data1$arm)
  fit_result <- elastic_net(resp = "tt_pfs", trt = "arm",
...
save(lasso1_HR1, file="Lasso/lasso1_HR1.RData")


## Lasso HR: scenario2
set.seed(0)

lasso2_HR1 <- matrix(NA,nrow=25,ncol=n_datasets)
...
save(lasso2_HR1, file="Lasso/lasso2_HR1.RData")


## Lasso HR: scenario3
set.seed(0)

lasso3_HR1 <- matrix(NA,nrow=25,ncol=n_datasets)
...
save(lasso3_HR1, file="Lasso/lasso3_HR1.RData")


## Lasso HR: scenario4
set.seed(0)

lasso4_HR1 <- matrix(NA,nrow=25,ncol=n_datasets)
...
save(lasso4_HR1, file="Lasso/lasso4_HR1.RData")


## Lasso HR: scenario5
set.seed(0)

lasso5_HR1 <- matrix(NA,nrow=25,ncol=n_datasets)
...
save(lasso5_HR1, file="Lasso/lasso5_HR1.RData")


## Lasso HR: scenario6
set.seed(0)

lasso6_HR1 <- matrix(NA,nrow=25,ncol=n_datasets)
...
save(lasso6_HR1, file="Lasso/lasso6_HR1.RData")
