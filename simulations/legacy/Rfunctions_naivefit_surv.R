...
generate_stacked_data <- function(base_model,subgroup_model,data,id_var="id",rename=T){
  ...
  d$subgroup <- factor(d$subgroup,levels=subgroup_names); d <- arrange(d,subgroup) # sort by "correct" variable order
...
calc_naive_subgroup_estimates <- function(base_model,subgroup_model,data, coxphw = FALSE){
  require(survival); require(coxphw); require(tidyverse); require(broom)
  ...
  if (coxphw==TRUE) {
    naive_estimates <- stacked_data %>% group_by(subgroup) %>% do(tidy(coxphw::coxphw(Surv(time,status)~arm, template = "AHR", data=.)))
  }
