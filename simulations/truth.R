...
save_file <- "scenarios/truth.Rdata"
...
if (file.exists(save_file)) {
  ...
  save(simul_parameter, file = save_file)
}

scenario_properties <- simul_parameter$true_subgroup_ahr |>
  as.data.frame() |>
  reshape(
    direction = "long",
    varying = list(seq_along(subgroup_names)),
    v.names = "true_ahr",
    timevar = "subgroup",
    idvar = "scenario",
    times = subgroup_names,
    ids = all_scenarios
  ) |>
  dplyr::mutate(
